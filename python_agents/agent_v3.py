...
# -*- coding: utf-8 -*-
"""
agent_v3.py — ИИ-агент v3.0: дневник + живой поток автономии.

Новое против v2.1:
- /free = НЕПРЕРЫВНЫЙ поток: выполнил действие → через секунду придумывает
  следующее; этапы связные (продолжает начатое, а не прыгает хаотично)
- упорство: при реальной ошибке пробует ДРУГИЕ способы (правило в промпте),
  формат сломался — до 5 переспросов вместо 3
- устойчивость к сети: сервер Ollama недоступен -> агент ждёт и
  переподключается 3 раза, трейсбэком больше не падаем
- то же ядро v2.1: дневник, уроки, шум-фильтр, анти-зацикливатель, CONFIRM=1

Запуск:
    python3 agent_v3.py
    AGENT_MODEL=mybrain python3 agent_v3.py
    CONFIRM=1 python3 agent_v3.py
...
BANNER = f"""============================================================
 ИИ-АГЕНТ v3.0 «НЕРАЗРЫВНЫЙ ПОТОК» | мозг: {MODEL}

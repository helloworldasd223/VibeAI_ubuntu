...
# -*- coding: utf-8 -*-
"""
agent_v4_1.py — ИИ-агент v4.1 «СОНАР»: видит интерфейс БЕЗ зрительной модели.

Сделан специально для ВМ с 4 ГБ RAM: вместо тяжёлой нейросети-зрения (~3.5 ГБ)
используется «сонар» — скрипт uitree.py, читающий дерево доступности GNOME
(AT-SPI). Он выдаёт все видимые кнопки/меню/поля с ТОЧНЫМИ координатами.
Память: ~0. Точность координат: 100% (не ±50 пикселей, как у глаз).

Опционально поддерживает настоящие глаза (AGENT_VISION=eyes): маленькая
зрительная модель (напр. Qwen2-VL-2B) — агент САМ выгружает мозг на время
взгляда (ollama stop + keep_alive=0), чтобы всё влезало в 4 ГБ.

Протокол действий (модель выбирает ОДНО действие за шаг):
  {"command": "shell-команда"}     — как раньше
  {"scan": ""}                     — сонар: список элементов экрана с координатами
  {"scan": "Settings"}             — сонар только по окну со словом в названии
  {"click": [x, y]}                — клик мышью в координаты
  {"see": "вопрос"}                — глаза (только если AGENT_VISION задан)
  {"done": "итог"}                 — задача выполнена

Переменные окружения:
    AGENT_MODEL=mybrain     текстовый мозг (по умолчанию mybrain)
    AGENT_VISION=eyes       зрительный мозг (по умолчанию ВЫКЛЮЧЕН)
    AGENT_UITREE=~/uitree.py  путь к сонару
    EYE_SWAP=0              НЕ выгружать мозг перед взглядом (по умолчанию выгружает)
    SCAN=always             после каждого действия автоматически смотреть сонаром
    CONFIRM=1               спрашивать разрешение на каждое действие

Требования: сеанс X11 (echo $XDG_SESSION_TYPE), xdotool, python3-pyatspi,
            включённая accessibility. Команды: задача | /free [N] | /memo | /exit
"""
import base64
import datetime
import json
...
BANNER = (f"============================================================\n"
          f" ИИ-АГЕНТ v4.1 «СОНАР» | мозг: {MODEL} | зрение: {VISION or 'ВЫКЛ (экономим RAM)'}"
...
def format_hint():
    h = ('Ответь СТРОГО одним JSON-объектом: {"command": "..."} / '
         '{"scan": ""} / {"click": [x, y]}')
    if VISION:
        h += ' / {"see": "..."}'
    return h + ' / {"done": "..."}.'
...
def stepper(task):
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        act_key = next((k for k in ("see", "click", "scan") if k in act), "command")
        ...
        if SCAN_ALWAYS and act_key in ("command", "click"):
            try:
                auto = run_scan("")
                out += "\nАВТОСКАН:\n" + auto[:1200]
            except Exception:
...
FREE_IDEAS = [
    'посмотри сонаром {"scan": ""} что на экране и сохрани список окон в файл ~/Desktop/sonar.txt',

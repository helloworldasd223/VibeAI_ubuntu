...
# -*- coding: utf-8 -*-
"""
agent_v2.py — ИИ-агент v2: дневник памяти + режим автономии.

По умолчанию агент ПАССИВЕН: выполняет только ваши запросы.
Команда /free включает автономный режим (сама придумывает цели и делает).
После перезапуска автономия ВЫКЛ, но память (~/agent_diary.md, ~/agent_notes.txt) остаётся.

Запуск:
    python3 agent_v2.py               # интерактив
    AGENT_MODEL=mybrain python3 agent_v2.py
    CONFIRM=1 python3 agent_v2.py     # спрашивать разрешение на каждую команду

Команды внутри:
    любая фраза          -> задача для агента
    /free [N]            -> автономия на N целей (по умолчанию 3)
    /memo                -> показать дневник
    /exit                -> выход (память сохраняется)
"""
import datetime
import json
import os
...
DIARY = os.path.join(HOME, "agent_diary.md")   # полный журнал эпизодов
NOTES = os.path.join(HOME, "agent_notes.txt")  # короткие уроки (пишет сама модель)
MAX_STEPS = 8          # шагов на одну задачу
MAX_OUT = 1500
FREE_BUDGET = 3
CONFIRM = os.environ.get("CONFIRM", "") == "1"

# Что агент НЕ выполнит, даже если модель придумает
...
- файлы: читать/писать в ~, ~/Desktop, /tmp (создание, правки, списки)
- системная информация: uname, nproc, free, df, ls, date, find
- GUI: запуск программ (например: nohup gnome-text-editor &), скриншоты (gnome-screenshot -f /tmp/screen.png)
- обои рабочего стола: gsettings set org.gnome.desktop.background picture-uri file:///usr/share/backgrounds/<файл>
- мышь/клавиатура: xdotool (ТОЛЬКО в сеансе X11; проверка: echo $XDG_SESSION_TYPE)
- интернет-текст: curl https://html.duckduckgo.com/html/?q=<запрос>
Не пытайся: ставить пакеты, работать с sudo, перезагружать, удалять системное."""
...
{"done": "итог по-русски"}        — когда задача выполнена и проверена командой

Правила:
- Одна простая команда за раз; после неё получишь реальный вывод в сообщении "OUTPUT:".
- НИКОГДА не пиши OUTPUT сам и не выдумывай результаты.
- Запрещены команды из стоп-листа безопасности и всё разрушительное.
- Если OUTPUT — ошибка, исправляй следующей командой, а не сдавайся.
- {"done": ...} — только когда OUTPUT последней команды подтверждает результат."""
...
- НЕ повторяй то, что уже делала (смотри дневник ниже)
...
BANNER = f"""============================================================
 ИИ-АГЕНТ v2 | мозг: {MODEL}
 По умолчанию пассивен: пиши задачу — выполню.
 Команды: /free [N] автономия | /memo дневник | /exit выход
============================================================"""
...
def add_note(line):
    ...
    if not line or line.startswith("-"):
        return
    lines = read_file(NOTES).splitlines()[-14:]  # помним последние 15 уроков
    lines.append(f"- {line}")
    try:
...
def system_prompt():
    n = notes()
    mem = ("\nТвой дневник уроков (ты писала их сама в прошлых сессиях — пользуйся):\n" + n) if n else ""
...
def reflect(task, ok, outcome):
    """Саморефлексия: модель сама формулирует урок эпизода и пишет в дневник уроков."""
    try:
        verdict = "успех" if ok else "неудача"
        line = chat([{"role": "system", "content": "Ты умеешь сжато формулировать выводы."},
...
def gen_goal():
    ...
    for _ in range(2):
        try:
            ...
            pass
    return random.choice(FREE_IDEAS)  # запасной вариант, если фантазия подвисла
